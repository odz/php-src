...
void SQLLog(char *filename) {
	...
	if (dbsock==NULL) {
		...
		if (dbsock==NULL) {
#endif
			php_error(E_WARNING, "Unable to connect to sql daemon");
			return;
		}
	}
#if MSQLLOGGING
	if (msqlSelectDB(dbsock,SQLLogDB)<0) {
		msqlClose(dbsock);
		php_error(E_WARNING, "Unable to select msql logging database (%s) - %s",SQLLogDB,msqlErrMsg);
		return;
	}
	...
	if (mysql_select_db(dbsock,SQLLogDB)<0) {
		mysql_close(dbsock);
		php_error(E_WARNING, "Unable to select mysql logging database (%s)",SQLLogDB);
		return;
	}
...
void sqlloadlastinfo(char *filename) {
	...
	if (mysql_select_db(dbsock,SQLLogDB)<0) {
		mysql_close(dbsock);
#endif
		php_error(E_WARNING, "Unable to select sql logging database (%s)",SQLLogDB);
		return;
	}
#if MSQLLOGGING
	msqlSetCurrent(dbsock,SQLLogDB);
