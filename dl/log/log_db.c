...
void php_log_db() {
	...
	/* construct the full path for the database */
        sprintf(path, "%s/%ld/%s.log", log_conf.dbm_log_dir, php_getuid(), log_stats.logfile);

	es = EG(error_reporting);
	EG(error_reporting) = 0;
	dbm = php_dbm_open(path, "w");
	EG(error_reporting) = es;

	if (!dbm) {
		dbm = php_dbm_open(path, "n");
		if (!dbm) {
			php_error(E_WARNING, "Unable to create %s", path);
			return;
		}
		...
		while(!php_dbm_insert(dbm, "first", key)) {
			retries++;
			if (retries > 20) {
				retries = 0;
				break;
	...
	ret=1;
	while(ret) {
		ret = php_dbm_insert(dbm, key, buf);
		if (ret) {
	...
	sprintf(buf,"%s %ld %ld %ld %s%c%s%c%s%c%s",
		...
		27,
		browser ? browser : "");
	php_dbm_replace(dbm, "last", buf);
	php_dbm_close(dbm);
#endif
}

void php_load_log_info_db() {
#if LOGGING
	dbm_info *dbm;
	char *value;
	char *s=NULL, *ss;
	...
	value = s = php_dbm_fetch(dbm, "last");
	...
	value = s = php_dbm_fetch(dbm, "first");
	if (!s) {
		...
		ret = 1;
		while (ret) {
			ret = php_dbm_insert(dbm,"first",key);
			retries++;
			if (retries>20) {
				retries = 0;
				break;
